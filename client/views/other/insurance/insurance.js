...
var App = React.createClass({
    componentDidMount: function () {
        $.get(URL.INSURANCE, function (result) {
            this.setState(result);
        }.bind(this));
    },
    handleClick: function (e) {
        if ($(e.target).hasClass('active')) {
            $(e.target).removeClass('active').siblings('.list-detail').slideUp(500);
        } else {
            $(e.target).addClass('active').siblings('.list-detail').slideDown(500);
        }
    },
