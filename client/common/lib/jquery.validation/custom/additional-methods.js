...
// Matches UK postcode. Does not match to UK Channel Islands that have their own postcodes (non standard UK)
     ...
     * Can component US Territories or not - default does not
     * Can component US Military postal abbreviations (AA, AE, AP) - default does not
     *
     ...
     *  }
     *
     *  Case insensitive, component territories but not military zones
     *  stateInput: {
     *     includeTerritories: true
     *  }
     *
     *  Only allow capital letters, component territories and military zones
     *  stateInput: {
     *     caseSensitive: true,
     *     includeTerritories: true,
...
//投资金额验证
    $.validator.addMethod("calcuInvestAmount", function (value, element) {
        var reg1 = /^[1-9]\d{0,7}$/;
        return this.optional(element) || (reg1.test(value));
    }, "投入金额为1-99999999的整数");

//预期年利率验证
    $.validator.addMethod("calcuRate", function (value, element) {
        var reg2 = /^0\S*$/;
        if (value >= 1 && value <= 100 && !reg2.test(value)) {
            return true;
        } else {
            return false;
        }
...
//投资期限验证
    $.validator.addMethod("calcuInvestTerm", function (value, element) {
        var reg3 = /^([1-4][0-9]|[1-9])$/;
        return this.optional(element) || (reg3.test(value));
    }, "投资期限为1-49的整数");

//借款金额验证
    $.validator.addMethod("borrowAmount", function (value, element) {
        return value % 1000 == 0 ? true : false;
    }, "借款金额必须是1000的倍数");

//身份证验证
    $.validator.addMethod("isIDCard", function (value, element) {
        return idCardNoValidate(value);
    }, "请输入正确的身份证号");


//是否包含特殊字符
    $.validator.addMethod("isSpecialCode", function (value, element) {
...
//手机号验证
    $.validator.addMethod("isMobilePhone", function (value, element) {
        var mobilePhone = /^1[3|4|5|7|8|][0-9]{9}$/;
...
//电话号验证（固话手机皆可通过）
    $.validator.addMethod("isPhone", function (value, element) {
        var mobilePhone = /^((\+86)?|\(\+86\)|\+86\s)0?([1-9]\d-?\d{6,8}|[3-9][13579]\d-?\d{6,7}|[3-9][24680]\d{2}-?\d{6})(-\d{3})?$/;
        return this.optional(element) || (mobilePhone.test(value));
    }, "请正确填写您的电话号");
...
//  固定电话号验证
    $.validator.addMethod("isTel", function (value, element) {
        var mobilePhone = /^(0[0-9]{2,3}\-)?([2-9][0-9]{6,7})+(\-[0-9]{1,4})?$/;
        return this.optional(element) || (mobilePhone.test(value));
    }, "请正确填写您的电话号");


//用户昵称验证
    $.validator.addMethod("nameWithoutNumStart", function (value, element) {
        var nickName = /^[^0-9](.)*$/;
...
//新密码的验证（密码中不能包含#$%^&.*~@以外的特殊字符）
    $.validator.addMethod("isNewPassword", function (value, element) {

        var realName = /^[a-zA-Z0-9#$%^&.*~@]+$/
        return this.optional(element) || (realName.test(value));
    }, "密码中不能包含特殊字符");
...
//两个输入框数值大小校验
    $.validator.addMethod("two_input_range", function (value, element, options) {
        var $fields = $(options[0], element.form),
            $fieldsFirst = $fields.eq(0),
            $fieldsSecond = $fields.eq(1),
            validator = $fieldsFirst.data("valid_input_range") ? $fieldsFirst.data("valid_input_range") : $.extend({}, this), isValid = $fields.filter(function () {
                    return validator.elementValue($fieldsSecond) - validator.elementValue($fieldsFirst) >= 0;
                }).length > 0;

        $fieldsFirst.data("valid_input_range", validator);
        // If element isn't being validated, run each require_from_group field's validation rules
        if (!$(element).data("being_validated")) {
            $fields.data("being_validated", true);
            $fields.each(function () {
